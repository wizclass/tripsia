...
if($w == '' || $w == 'a' || $w == 'r') {
    ...
    $insert_qa_file1 = isset($upload[0]['file']) ? $upload[0]['file'] : '';
    $insert_qa_source1 = isset($upload[0]['source']) ? $upload[0]['source'] : '';
    $insert_qa_file2 = isset($upload[1]['file']) ? $upload[1]['file'] : '';
    $insert_qa_source2 = isset($upload[1]['source']) ? $upload[1]['source'] : '';
    ...
    if(!$upload[0]['file'] && !$upload[0]['del_check']) {
        $upload[0]['file'] = $write['qa_file1'];
        $upload[0]['source'] = $write['qa_source1'];
    }

    if(!$upload[1]['file'] && !$upload[1]['del_check']) {
        $upload[1]['file'] = $write['qa_file2'];
        $upload[1]['source'] = $write['qa_source2'];
    }
    ...
    $sql = " update {$g5['qa_content_table']}
                set qa_email    = '$qa_email',
                    ...
                    qa_subject  = '$qa_subject',
                    qa_content  = '$qa_content',
                    qa_file1    = '{$upload[0]['file']}',
                    qa_source1  = '{$upload[0]['source']}',
                    qa_file2    = '{$upload[1]['file']}',
                    qa_source2  = '{$upload[1]['source']}',
                    qa_1        = '$qa_1',
                    qa_2        = '$qa_2',
                    qa_3        = '$qa_3',
                    qa_4        = '$qa_4',
...
if($w == 'a')
    $result_url = G5_ADMIN_BBS_URL.'/qaview.php?qa_id='.$qa_id."&".$qstr;
else if($w == 'u' && $write['qa_type'])
    $result_url = G5_ADMIN_BBS_URL.'/qaview.php?qa_id='.$write['qa_parent']."&".$qstr;
else
    $result_url = G5_ADMIN_BBS_URL.'/qalist.php?'.preg_replace('/^&amp;/', '?', $qstr);
