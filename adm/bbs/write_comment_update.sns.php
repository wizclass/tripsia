...
if (! empty($_POST['facebook_checked'])) {
    ...
    if ($user) {
        try {
            $link = get_pretty_url($bo_table, $wr['wr_parent'], '&#c_'.$comment_id,'',G5_ADMIN_BBS_URL);
            $attachment = array(
...
if (! empty($_POST['twitter_checked'])) {
    ...
    if ( !(empty($_SESSION['access_token']) || empty($_SESSION['access_token']['oauth_token']) || empty($_SESSION['access_token']['oauth_token_secret'])) ) {
        $comment_url = get_pretty_url($bo_table, $wr['wr_parent'], '&#c_'.$comment_id,'',G5_ADMIN_BBS_URL);
