...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if (!strstr($row['wr_option'], 'secret') ||
        ...
        if(!get_session($ss_name))
            $list[$i]['content'] = '<a href="'.G5_ADMIN_BBS_URL.'/password.php?w=sc&amp;bo_table='.$bo_table.'&amp;wr_id='.$list[$i]['wr_id']."&".$qstr.'" class="s_cmt">댓글내용 확인</a>';
        else {
            $list[$i]['content'] = conv_content($row['wr_content'], 0, 'wr_content');
            $list[$i]['content'] = search_font($stx, $list[$i]['content']);
        }
    ...
    if ($is_comment_write || $is_admin)
    {
        ...
        {
            if (!$row['mb_id']) {
                $list[$i]['del_link'] = G5_ADMIN_BBS_URL.'/password.php?w=x&amp;bo_table='.$bo_table.'&amp;comment_id='.$row['wr_id'].'&amp;page='.$page."&".$qstr;
                $list[$i]['is_del']   = true;
            }
        }
