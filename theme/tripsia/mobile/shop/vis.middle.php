...
function mdlslide() {
	...
	var $w = $(window).width() - 20; // 20은 padding:10px;
	var $h = parseInt($w * $dmt);
	$('#mdl_slide, .mdlBx').width($w);
	$('#mdl_slide, .mdl_Wrp, .mdlBx').height($h);
...
<style type="text/css">
/* mdl_slide common */
#mdl_slide {position:relative;padding:0 !important;width:100%;height:295px;margin:0 auto;}
#mdl_slide div.mdl_Wrp {height:295px;overflow:hidden;}
#mdl_slide div.mdlBx {float:left;position:relative;width:100%;height:295px;margin:0;padding:0;}
...
#mdl_slide span.pagination {position:absolute;left:50%;margin-left:-150px;width:300px;height:40px;line-height:40px;text-align:center;}
#mdl_slide span.pagination a {display:inline-block;*display:inline;*zoom:1;width:15px;height:15px;margin:0 5px;background-color:#bfbfbf;border-radius:50%;}
...
<!--▼ mdl_slide ▼-->
	...
	<div class="mdl_Wrp">
	<?
		$qry = sql_query(" select * from g5_shop_banner where bn_position = 'mb_middle' order by bn_order asc ");
