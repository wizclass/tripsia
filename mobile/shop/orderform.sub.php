...
<div id="sod_frm" class="sod_frm_mobile">
    <form name="forderform" method="post" action="<?php echo $order_action_url; ?>" autocomplete="off">
        ...
        <input type="hidden" name="od_token_price" id="od_token_price">
        <?php echo $content; ?>
        ...
        <section id="sod_frm_pay">
            ...
            if ($is_kakaopay_use || $default['de_bank_use'] || $default['de_coin_use'] || $default['de_vbank_use'] || $default['de_iche_use'] || $default['de_card_use'] || $default['de_hp_use'] || $default['de_easy_pay_use'] || is_inicis_simple_pay()) {
            ...
            if ($default['de_coin_use']) {
                $multi_settle++;
                echo '<li><input type="radio" id="od_settle_coin" name="od_settle_case" value="코인" ' . $checked . '> <label for="od_settle_coin" class="lb_icon  bank_icon">' . $token_symbol . ' 결제</label></li>' . PHP_EOL;
                $checked = '';
            }
            ...
            if ($default['de_bank_use']) {
                ...
                echo '<div id="settle_bank" style="display:none"><div class="wallet_address_title">입금계좌</div>';
            ...
            if ($default['de_coin_use'] > 1) {
                ...
                if (count($str_2) <= 1) {
                    ...
                    $bank_account_2 = '<select name="od_coin_account" id="od_coin_account">' . PHP_EOL;
                    $bank_account_2 .= '<option value="">선택하십시오.</option>';
                    for ($i = 0; $i < count($str_2); $i++) {
                        //$str_2[$i] = str_replace("\r", "", $str_2[$i]);
                        $str_2[$i] = trim($str_2[$i]);
                        $bank_account_2 .= '<option value="' . $str_2[$i] . '">' . $str_2[$i] . '</option>' . PHP_EOL;
                    }
                    $bank_account_2 .= '</select>' . PHP_EOL;
                }
                echo '<div id="settle_coin" style="display:none"><div class="wallet_address_title">지갑주소</div>';
                echo '<label for="od_coin_account" class="sound_only">입금할 계좌</label>';
                echo "<div class='wallet_address'>".$bank_account_2."</div>";
...
<script>
    ...
    $(function() {
        ...
        $("#od_settle_coin").on("click", function() {
            $("[name=od_coin_deposit_name]").val($("[name=od_name]").val());
            $("#settle_coin").show();
            $("#settle_bank").hide();
            $("#show_req_btn").css("display", "none");
            $("#show_pay_btn").css("display", "inline");
    ...
    function pay_approval() {
        ...
        if (settle_method != "무통장" || settle_method != "코인") {
    ...
    function orderfield_check(f) {
        ...
        var od_settle_coin = document.getElementById("od_settle_coin");
        if (od_settle_coin) {
            if (od_settle_coin.checked) {
                check_field(f.od_coin_deposit_name, "입금자명을 입력하세요.");
            }
        }
